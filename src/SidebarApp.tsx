...
interface GenerateRecordPayload {
  recordId: string;
...
const GENERATE_REQUEST_BATCH_SIZE = 10;
const MAX_IMAGE_URLS_PER_VARIABLE = 5;
...
async function* getRecordIdBatches(table: ITable, batchSize: number): AsyncGenerator<string[]> {
  let pending: string[] = [];
  ...
  while (true) {
    ...
    pending.push(...page.recordIds);
    while (pending.length >= batchSize) {
      yield pending.slice(0, batchSize);
...
function normalizeAttachmentConfigs(input: unknown): Record<string, AttachmentFieldConfig> {
  const result: Record<string, AttachmentFieldConfig> = {};
  if (!input || typeof input !== "object") {
    return result;
  }

  for (const [variable, raw] of Object.entries(input)) {
    if (!raw || typeof raw !== "object") {
      continue;
    }
    const value = raw as Partial<AttachmentFieldConfig>;
    const numericWidth = Number(value.imageWidth);
    const imageWidth = Number.isFinite(numericWidth)
      ? Math.max(0, Math.min(2000, Math.floor(numericWidth)))
      : DEFAULT_IMAGE_WIDTH;
    result[variable] = {
      imageWidth,
      widthMode: value.widthMode === "custom" ? "custom" : "preset",
    };
  }
...
export default function App({ initialAuthUser = null }: SidebarAppProps) {
  ...
  const [authSession, setAuthSession] = useState<AuthSession>({
    ...
    isAuthenticated: Boolean(initialAuthUser),
  });
  const [authLoading, setAuthLoading] = useState(!initialAuthUser);
  ...
  const fetchLinkedTableFields = useCallback(async (tableId: string): Promise<FieldMetaLite[]> => {
    ...
    const task = (async () => {
      try {
        ...
        setNotice({
          type: "error",
          text: `读取关联表字段失败：${toErrorMessage(error)}`,
        });
        return [];
      }
    })();
  ...
  }, [checkAuthSession, initialAuthUser]);
  ...
  const applySavedPayload = useCallback((config: SavedConfigPayload) => {
    ...
    setAttachmentConfigs(normalizeAttachmentConfigs(config.attachmentConfigs));
  ...
  const handleSelectSavedTemplate = useCallback(async (item: SavedTemplateItem) => {
    ...
    try {
      ...
      setNotice({ type: "error", text: "模板加载失败，请稍后重试。" });
    } finally {
      setIsExtracting(false);
  ...
  const collectVariablesForRecords = useCallback(
    ...
    ): Promise<GenerateRecordPayload[]> => {
      if (!table) {
        throw new Error("当前数据表未就绪。");
      }
      const records: GenerateRecordPayload[] = [];
      ...
      for (let index = 0; index < recordIds.length; index += 1) {
        ...
        await Promise.all(
          variables.map(async (variable) => {
            ...
            try {
              if (LINK_FIELD_TYPES.has(field.type as FieldType)) {
                ...
                if (isLinkedFieldAttachment) {
                  ...
                  for (const rid of link.recordIds) {
                    if (allImageUrls.length >= MAX_IMAGE_URLS_PER_VARIABLE) {
                    ...
                    try {
                      ...
                      if (attachments?.length) {
                        const imageAttachmentsInCell = attachments.filter((a) => a.type?.startsWith("image/"));
                        const remainingSlots = MAX_IMAGE_URLS_PER_VARIABLE - allImageUrls.length;
                        const imageAttachments = imageAttachmentsInCell.slice(0, remainingSlots);
                        if (imageAttachments.length > 0) {
                          ...
                          allImageUrls.push(...urls.filter(Boolean).slice(0, remainingSlots));
                          if (imageAttachmentsInCell.length > remainingSlots) {
                            imageLimitReached = true;
                          }
                        }
                      }
                    } catch (error) {
                      recordWarnings.push(`变量「${variable}」关联附件读取失败：${toErrorMessage(error)}`);
                    }
                  }

                  const imageUrls = allImageUrls.filter(Boolean);
                  if (imageUrls.length > 0) {
                    if (imageLimitReached) {
                      recordWarnings.push(`变量「${variable}」图片超过 ${MAX_IMAGE_URLS_PER_VARIABLE} 张，仅处理前 ${MAX_IMAGE_URLS_PER_VARIABLE} 张`);
                    }
                    const attachConfig = attachmentConfigs[variable];
                    imageVars[variable] = {
                      urls: imageUrls.slice(0, MAX_IMAGE_URLS_PER_VARIABLE),
                  ...
                  for (const rid of link.recordIds) {
                    try {
                      ...
                      recordWarnings.push(`变量「${variable}」关联字段读取失败：${toErrorMessage(error)}`);
                    }
                  }
                ...
                const tokens = imageAttachments.slice(0, MAX_IMAGE_URLS_PER_VARIABLE).map((a) => a.token);
                const urls = (await table.getCellAttachmentUrls(tokens, fieldId, recordId)).filter(Boolean);
                if (imageAttachments.length > MAX_IMAGE_URLS_PER_VARIABLE) {
                  recordWarnings.push(`变量「${variable}」图片超过 ${MAX_IMAGE_URLS_PER_VARIABLE} 张，仅处理前 ${MAX_IMAGE_URLS_PER_VARIABLE} 张`);
                }
                if (urls.length > 0) {
                  imageVars[variable] = {
                    urls: urls.slice(0, MAX_IMAGE_URLS_PER_VARIABLE),
                    width: attachConfig?.imageWidth ?? DEFAULT_IMAGE_WIDTH,
                  };
                } else {
                  textVars[variable] = "";
              ...
              recordWarnings.push(`变量「${variable}」读取失败：${toErrorMessage(error)}`);
              textVars[variable] = "";
            }
          }),
        );

        const entry: GenerateRecordPayload = { recordId, variables: textVars };
        ...
        if (titleField) {
          try {
            ...
            recordWarnings.push(`标题字段读取失败：${toErrorMessage(error)}`);
          }
        }

        if (recordWarnings.length > 0) {
          entry.warnings = recordWarnings;
        }
  ...
  const executeGenerate = useCallback(
    async (generateMode: GenerateMode) => {
      ...
      let selectedTargetRecordIds: string[] = [];
      let selectedRecordBatches: string[][] = [];
      let firstAllBatch: string[] | null = null;
      let allRecordIterator: AsyncIterator<string[]> | null = null;

      if (generateMode === "selected") {
        selectedTargetRecordIds = [...selectedRecordIds];
        selectedRecordBatches = chunkArray(selectedTargetRecordIds, GENERATE_REQUEST_BATCH_SIZE);
      } else {
        allRecordIterator = getRecordIdBatches(table, GENERATE_REQUEST_BATCH_SIZE)[Symbol.asyncIterator]();
        const first = await allRecordIterator.next();
        firstAllBatch = first.done ? null : first.value;
      }
      if (generateMode === "selected" ? selectedTargetRecordIds.length === 0 : !firstAllBatch?.length) {
        setNotice({
          type: "error",
          text:
      ...
      const initialTotal = generateMode === "selected"
        ? selectedTargetRecordIds.length
        : firstAllBatch?.length ?? GENERATE_REQUEST_BATCH_SIZE;
      ...
      try {
        ...
        const generatedResults: GenerateApiResult[] = [];
        let totalRecords = initialTotal;
        let preparedCount = 0;
        let batchIndex = 0;

        async function* iterateRecordBatches(): AsyncGenerator<string[]> {
          if (generateMode === "selected") {
            for (const batch of selectedRecordBatches) {
              yield batch;
            }
            return;
          }

          if (firstAllBatch?.length) {
            yield firstAllBatch;
          }
          if (!allRecordIterator) {
            return;
          }
          while (true) {
            const next = await allRecordIterator.next();
            if (next.done) {
              break;
            }
            yield next.value;
        ...
        for await (const batchRecordIds of iterateRecordBatches()) {
          batchIndex += 1;
          totalRecords = Math.max(totalRecords, preparedCount + batchRecordIds.length);
          const batchLabel = generateMode === "selected"
            ? `${batchIndex}/${selectedRecordBatches.length}`
            : `${batchIndex}`;
          const batch = await collectVariablesForRecords(
            batchRecordIds,
            titleFieldId || undefined,
            (done) => {
              const overallDone = preparedCount + done;
              const progressTotal = Math.max(totalRecords, overallDone);
              const prepCap = Math.max(1, Math.floor(progressTotal * 0.45));
              const progressCount = Math.max(
                0,
                Math.min(
                  progressTotal,
                  Math.floor((overallDone / Math.max(1, progressTotal)) * prepCap),
                ),
              );
              setGenerateProgress({
                visible: true,
                ...
                phase: `正在准备变量 (${batchLabel})...`,
              });
            },
          );
          preparedCount += batch.length;
          totalRecords = Math.max(totalRecords, preparedCount);
          const localWarningsByRecord = new Map(
            batch.map((record) => [record.recordId, record.warnings ?? []]),
          );
          const cloudBase = Math.floor(totalRecords * 0.45);
        ...
        for (let index = 0; index < generatedResults.length; index += 1) {
          ...
          setGenerateProgress({
            visible: true,
            total: totalRecords,
            completed: Math.max(1, Math.min(totalRecords, Math.floor(totalRecords * 0.85) + index + 1)),
            phase: "正在写回结果...",
          });
        }
        ...
        const warningCount = writeBackResults.filter(
          (item) => (item.warnings?.length ?? 0) > 0,
        ).length;
        const successCount = writeBackResults.filter(
          (item) => item.status === "success" && (item.warnings?.length ?? 0) === 0,
        ).length;
        setNotice({
          type: failedCount > 0 ? "error" : warningCount > 0 ? "info" : "success",
          text: `已完成：成功 ${successCount} 条，需注意 ${warningCount} 条，失败 ${failedCount} 条。`,
        });
        await refreshTableContext();
      } catch (error) {
        setNotice({
                            ...
                            <div className="w-[150px] shrink-0">
                              <MappingSelect
                                ...
                                onChange={(val) => {
                                  ...
                                  const numericPreset = Number(val);
                                  const preset = Math.max(0, Math.min(2000, Number.isFinite(numericPreset) ? numericPreset : DEFAULT_IMAGE_WIDTH));
                                  setAttachmentConfigs((prev) => ({
                                    ...prev,
                            ...
                            {attachConfig.widthMode === "custom" && (
                              <div className="flex items-center gap-1.5 shrink-0">
                                <input
                                  ...
                                  value={String(attachConfig.imageWidth ?? "")}
                                  placeholder="400"
                                  onChange={(e) => {
              ...
              <>
                ...
                <div className="px-5 py-6 space-y-3">
                  ...
                  {results.map((result, idx) => {
                    ...
                    const resultState = result.status === "failed" ? "failed" : hasWarnings ? "warning" : "success";
                    const statusClass = resultState === "success"
                      ? "text-[#34c759]"
                      : resultState === "warning"
                        ? "text-[#d97917]"
                        : "text-[#f54a45]";
                    const cardClass = resultState === "warning"
                      ? "border-[#f7c873] bg-[#fff9ec] dark:bg-[#3a2a12]/30 dark:border-[#8a641b]"
                      : resultState === "failed"
                        ? "border-[#ffd6d3] bg-[#fff1f0] dark:bg-[#3a1718]/30 dark:border-[#7a2e2e]"
                        : "border-[#dee0e3] bg-[#f5f6f7] dark:bg-gray-800/30 dark:border-gray-800";
                    const warningText = formatWarnings(result.warnings);
                    const detailText = [result.error, warningText]
                      .filter(Boolean)
                      .join("；") ||
                      (result.replacedBlocks ? `替换 ${result.replacedBlocks} 处` : "—");
                    return (
                      <div
                        key={`${result.recordId}-${idx}`}
                        className={`text-[13px] border rounded-[8px] p-3 ${cardClass}`}
                      >
                        <div className="flex justify-between items-center gap-3 mb-1.5">
                          <span className="text-[#8f959e] font-mono truncate max-w-[60%]">
                            ID: {result.recordId}
                          </span>
                          <span className={`flex items-center gap-1 shrink-0 ${statusClass}`}>
                            {resultState === "success" ? (
                            ...
                            <span>
                              {resultState === "success" ? "成功" : resultState === "warning" ? "需注意" : "失败"}
                            </span>
                          </span>
                        </div>
                        <div className="flex justify-between items-start gap-3">
                          {result.docUrl ? (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="text-[#3370ff] hover:underline flex items-center gap-1 font-medium shrink-0"
                            >
                              查看文档{" "}
                          ...
                          <span
                            className="text-[#5f6670] dark:text-gray-300 break-words min-w-0 text-right"
                            title={detailText}
                          >
                            {detailText}
                          </span>
