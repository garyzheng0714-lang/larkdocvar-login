...
| `download.fileBase64` | string | 仅**单份生成 / 同步批量**在请求 `output.includeFileBase64=true` 时返回，用于侧边栏把生成文件写回附件字段；业务系统不应长期保存。**异步任务不返回该字段**（即使请求了也不返回），请用 `download.url` 下载。 |
...
飞书多维表格侧边栏内的请求仍应带 `credentials: include`。侧边栏可以额外发送以下 `X-Bitable-*` 上下文头，供排查宿主 Base/Table/Tenant 和后续可信登录链路使用，但这组头来自客户端，不能单独作为认证凭据，不能用来绑定模板创建者或读取 `private` 模板：

| 名称 | 类型 | 必填 | 描述 |
|---|---|---|---|
| `X-Bitable-Open-Id` | string | 否 | 当前侧边栏用户的 open_id，来自 Base JS SDK；仅作上下文线索，不等同登录用户。 |
| `X-Bitable-Base-User-Id` | string | 否 | 当 `X-Bitable-Open-Id` 不可用时的宿主用户线索；仅作上下文线索。 |
| `X-Bitable-Base-Id` | string | 否 | 当前多维表格 Base ID；仅用于上下文校验和排查。 |
| `X-Bitable-Table-Id` | string | 否 | 当前数据表 ID；仅用于上下文校验和排查。 |
| `X-Bitable-Tenant-Key` | string | 否 | 当前飞书租户；仅用于上下文校验和排查。 |

服务端如果已经识别出登录会话，会使用登录会话里的用户身份；业务系统、外部脚本和第三方服务应继续使用 `Authorization` 或 `x-api-key`。不要把 `X-Bitable-*` 当作 API Key、Bearer token 或用户登录态。
...
| `output.includeFileBase64` | boolean | 否 | 是否在 `download.fileBase64` 返回文件内容。只建议侧边栏附件写回使用。**仅单份生成 / 同步批量有效；异步任务会忽略该字段**（大批量内联 base64 会撑爆内存/存储），异步一律用 `download.url` 下载。 |
