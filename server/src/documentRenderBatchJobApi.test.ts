...
async function createTemplate(
  baseUrl: string,
  input: { templateId?: string; visibility?: 'private' | 'shared' } = {},
): Promise<void> {
  const response = await fetch(`${baseUrl}/api/v1/document-templates`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({
      templateId: input.templateId || 'fbiftemp_20260512_001',
      name: '通用合同模板',
...
function withDocumentRenderApiKey(value: string): () => void {
  const previous = process.env.DOCUMENT_RENDER_API_KEY;
  process.env.DOCUMENT_RENDER_API_KEY = value;
  return () => {
...
test('private 模板不能被无权限用户通过生成、批量或异步任务绕过读取', async () => {
  const restorePrivateUrls = withPrivateTemplateUrls();
  const restoreApiKey = withDocumentRenderApiKey('test-docx-key');
  const api = await startServer();
  try {
    const templateId = 'private_render_tpl';
    await createTemplate(api.baseUrl, { templateId, visibility: 'private' });

    const singleResponse = await fetch(`${api.baseUrl}/api/v1/document-renders`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        template: { format: 'docx', templateId },
        variables: { 客户名称: '上海测试科技有限公司', 金额: '12800 元' },
      }),
    });
    const singleBody = await singleResponse.json() as any;
    assert.equal(singleResponse.status, 400);
    assert.match(singleBody.error, /没有权限使用此模板/);

    const batchResponse = await fetch(`${api.baseUrl}/api/v1/document-renders/batch`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        template: { format: 'docx', templateId },
        records: [
          { recordId: 'rec_private', variables: { 客户名称: '上海测试科技有限公司', 金额: '12800 元' } },
        ],
      }),
    });
    const batchBody = await batchResponse.json() as any;
    assert.equal(batchResponse.status, 400);
    assert.match(batchBody.error, /没有权限使用此模板/);

    const jobResponse = await fetch(`${api.baseUrl}/api/v1/document-render-jobs`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        template: { format: 'docx', templateId },
    ...
    assert.match(jobBody.error, /没有权限使用此模板/);

    const adminResponse = await fetch(`${api.baseUrl}/api/v1/document-renders`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'x-api-key': 'test-docx-key' },
      body: JSON.stringify({
        template: { format: 'docx', templateId },
        variables: { 客户名称: '上海测试科技有限公司', 金额: '12800 元' },
      }),
  ...
  try {
    ...
    const response = await fetch(`${api.baseUrl}/api/v1/document-renders/batch`, {
      ...
      body: JSON.stringify({
        ...
        records: [
          { recordId: 'rec_blank', variables: { 客户名称: '北京测试科技有限公司' } },
        ],
      }),
    });
    const body = await response.json() as any;
    ...
    assert.equal(body.records[0].ok, true);
    assert.deepEqual(body.records[0].variables.missing, ['金额']);
  } finally {
    restore();
    await api.close();
  }
