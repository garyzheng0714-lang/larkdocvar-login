...
test('飞书客户端内免登配置接口只返回当前入口 app_id', async () => {
  await withEnvAsync({
    FEISHU_FBIF_APP_ID: 'cli_fbif',
    FEISHU_FBIF_APP_SECRET: 'fbif_secret',
    FEISHU_FUDE_APP_ID: 'cli_fude',
    FEISHU_FUDE_APP_SECRET: 'fude_secret',
  }, async () => {
    const app = express();
    registerOAuthRoutes(app);
    ...
    await withTestServer(app, async (baseUrl) => {
      const response = await fetch(`${baseUrl}/api/auth/feishu/fude/client-config`);
      const body = (await response.json()) as { ok?: boolean; app_id?: string };

      assert.equal(response.status, 200);
      assert.equal(body.ok, true);
      assert.equal(body.app_id, 'cli_fude');
    });
  });
});

test('外部 OAuth handoff 领取接口已停用，避免用 state 轮询领取 session', async () => {
  await withEnvAsync({
    FEISHU_FBIF_APP_ID: 'cli_fbif',
    FEISHU_FBIF_APP_SECRET: 'fbif_secret',
  }, async () => {
    ...
    await withTestServer(app, async (baseUrl) => {
      const startResponse = await fetch(`${baseUrl}/api/auth/feishu/fbif/start`);
      const statusResponse = await fetch(`${baseUrl}/api/auth/feishu/fbif/login-status?state=abc`);
      const startBody = (await startResponse.json()) as { ok?: boolean; authorizeUrl?: string };
      const statusBody = (await statusResponse.json()) as { ok?: boolean; session_token?: string };

      assert.equal(startResponse.status, 410);
      assert.equal(startBody.ok, false);
      assert.equal(startBody.authorizeUrl, undefined);
      assert.equal(statusResponse.status, 410);
      assert.equal(statusBody.ok, false);
      assert.equal(statusBody.session_token, undefined);
    });
  });
});

test('登录配置缺失时 API 不向终端返回内部环境变量名', async () => {
  await withEnvAsync({}, async () => {
    const app = express();
    registerOAuthRoutes(app);

    await withTestServer(app, async (baseUrl) => {
      const response = await fetch(`${baseUrl}/api/auth/feishu/fbif/client-config`);
      const body = (await response.json()) as { ok?: boolean; error?: string };

      assert.equal(response.status, 500);
      assert.equal(body.ok, false);
      assert.equal(body.error, '登录服务配置不完整，请联系管理员。');
      assert.equal(body.error?.includes('FEISHU_'), false);
    });
  });
});

test('飞书客户端内免登 code 为空时直接拒绝，不进入 OAuth 换 token', async () => {
  await withEnvAsync({
    FEISHU_FBIF_APP_ID: 'cli_fbif',
    FEISHU_FBIF_APP_SECRET: 'fbif_secret',
  }, async () => {
    ...
    try {
      ...
      await withTestServer(app, async (baseUrl) => {
        const response = await fetch(`${baseUrl}/api/auth/feishu/fbif/client-code`, {
          method: 'POST',
