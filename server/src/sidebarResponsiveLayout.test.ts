...
test('保存模板前先检查可信会话，避免文件处理完成后才报登录失败', () => {
  ...
  // 免登失败后仍提供扫码兜底（重构后文案：按钮「扫码备用登录」）。
  assert.match(newTemplateScreenSource, /扫码备用登录/);
...
test('真实插件入口必须先接上可信会话，不能进主界面后才在保存时报登录失败', () => {
  ...
  // 重构后入口模型：端内免登为主（主按钮「重新尝试飞书免登」）+ 扫码兜底。
  assert.match(appSource, /重新尝试飞书免登/);
  assert.match(appSource, /扫码登录/);
  // 免登优先：入口流程先尝试 client-code 免登，再退到扫码兜底。
  assert.ok(appSource.indexOf('const attempt = await tryFeishuClientTrustedLogin()') < appSource.indexOf('const startQrLogin'));
